...
- **Estadísticas**: 6 tiles clickeables (mismo look `.skill-btn`/`.skill-mod` de `script.js`) — Vida (modal con barra de color por umbral + ±1/±5 HP + ±1 Vida Máx., igual que `combate.js`), CA (±1 + "Restaurar valor original"), Velocidad e Iniciativa (modal genérico de un campo), **Acciones/Turno** (modal genérico numérico — al guardar también ajusta `turnoActual.accion`: si estaba al máximo anterior lo sube al nuevo máximo, si no, lo recorta si hace falta) y **Acciones Legendarias** (abre `#legendarias-modal`: el mismo checkbox + cantidad que la creación; muestra "No" o "N/ronda").
